...
@private
    ...
    MetalImageCustomFilter    *_videoFilter;
}
...
- (void)viewDidLoad {
    ...
    renderView2.filterDelegate      = (MetalImageVideo*)_videoFilter;
    ...
    [_videoFilter configure:&plinestate];
}
...
- (void)gameloop
{
    ...
    assert([self.view isKindOfClass:[MetalImageCustomView class]]);
...
- (void)setPaused:(BOOL)pause
{
    ...
    if(_dispTimer)
    {
        ...
        if(pause == YES)
        {
            ...
            [(MetalImageCustomView *)self.view releaseTextures];
        }
        else
        {
            _gameLoopPaused = pause;
...
-(void)initCommon
{
    _videoFilter = [MetalImageVideo new];
